...
if __name__ == '__main__':
    ...
    samples = util.parse_car_csv(voc_car_train_dir)     # all of car_image
    ...
    for sample_name in samples:
        ...
        print('positive_annotation_path', positive_annotation_path)
        positive_bndboxes = np.loadtxt(positive_annotation_path, dtype=np.int, delimiter=' ')
        print('positive_bndboxes', len(positive_bndboxes))
        
        
        # 提取标注边界框
        gt_annotation_path = os.path.join(gt_annotation_dir, sample_name + '.xml')  # Groun Truth
